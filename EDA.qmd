...
summary(data)
```
We then create some exploratory plots comparing the distributions of all variables compared to our response, noting some potential variable of interest in high blood pressure, high cholesterol, smoker, general health, education, income, BMI, and Physical Health, based on how each of these count variables appears to change the regular proportions expected across a large number of non-diabetics and a smaller number of diabetics, and how each of the numeric variables appears to be shifted in distribution.
```{r}
