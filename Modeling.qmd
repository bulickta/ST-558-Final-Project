...
We fit three models as discussed - the full model, the reduced model, and the strict model. As calculating the log loss for the test data set also takes a few steps to complete, we put it in a function for ease of reproduction across all our models.
```{r}
...
logLoss <- function(model,testSet){
  predvals <- predict(model,newdata=test,type="raw")
  predprobs <- predict(model,newdata=test,type="prob")
  lossTest <- data.frame(obs=test$Diabetes_binary,pred=predvals,NonDiabetic=predprobs[,1],Diabetic=predprobs[,2])
...
In this case, we determine the best tree model to be the strict model variables, with a best tuning parameter cp=0.
```{r}
...
Finally, we can compare our best logistic regression model, tree model, and forest model to determine which is most effective, and we see that X minimizes log loss across all options
```{r}
logLoss(logModel1,test)
logLoss(treeModel3,test)
